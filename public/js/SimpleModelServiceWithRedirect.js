...
function simpleModelServiceWithRedirect(urlLink, returnUrlLink) 
{
    var URLLINK = urlLink;
        ...
        for (let i = 0; i < editButtons.length; i++) {
            editButtons[i].addEventListener('click', function(e) {
                ...
                for(let i = 0; i < elementsArr.length; i++) {
                    ...
                    let inputValue = elementsArr[i].value;
                    if(inputValue.length != 0) {
                        continue;
                    } else if(inputType == "text") {
                        elementsArr[i].value = parentRow.children[i].innerHTML;
                    } else if (inputType == "checkbox") {
                        elementsArr[i].checked = parentRow.children[i].querySelector("input").checked;
                    }
                }
            })
        }
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (!$.isEmptyObject(data.error)) {
                        ...
                        window.location.href = RETURNURLLINK;
                    }
                },
    ...
    function generateSubmitButtonForCreateEvent(submitButtonForCreateId, modalId , ...inputElementsIdArr) {
        var elementsArr = []
        for(let i = 0; i < inputElementsIdArr.length; i++) {
            elementsArr.push(document.getElementById(inputElementsIdArr[i]))
        }
